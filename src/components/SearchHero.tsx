...
import { FaBolt, FaMapMarkerAlt, FaCity, FaMap, FaChevronDown, FaCog, FaArrowRight } from 'react-icons/fa';
...
">
            ...
            <div id="filtro-busqueda" className="mt-24 lg:mt-32 max-w-5xl mx-auto relative z-20">
                <form
                    ...
                    className="bg-white/95 backdrop-blur-2xl border border-white/60 p-3 md:p-4 rounded-[32px] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.12)] flex flex-col w-full"
                >
                    <div className="flex flex-col md:flex-row gap-3 w-full items-center">
                        ...
                        <div className="flex w-full md:w-auto mt-2 md:mt-0 gap-2">
                            <button
                                ...
                                className={`p-5 rounded-[24px] transition-colors duration-300 flex items-center justify-center ${showAvanzados ? 'bg-[#a1824a] text-white' : 'bg-[#f9f8f6] hover:bg-[#f0eee9] text-[#a1824a]'}`}
                                title="Filtros avanzados"
                            >
                                <FaCog size={18} className={`transition-transform duration-500 ${showAvanzados ? 'rotate-90' : 'rotate-0'}`} />
                            </button>
                            <button
                                type="submit"
                                className="group flex-1 md:flex-none bg-[#630d16] hover:bg-black text-white font-sans font-black text-[13px] uppercase tracking-[0.2em] py-5 px-12 rounded-[24px] transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-1 flex items-center justify-center gap-3"
                            >
                                Explorar
                                <FaArrowRight size={12} className="opacity-80 transition-transform duration-300 group-hover:translate-x-1.5" />
                            </button>
                        </div>

                    </div>

                    <div className={`transition-all duration-500 overflow-hidden ${showAvanzados ? 'max-h-40 opacity-100 mt-3 md:mt-4' : 'max-h-0 opacity-0 mt-0'}`}>
                        <div className="flex flex-col md:flex-row gap-3 w-full pt-3 border-t border-gray-100">

                            {/* Tipo de Propiedad */}
                            <div className="flex-1 relative group bg-[#f9f8f6] rounded-[24px] hover:bg-[#f0eee9] w-full transition-colors duration-300">
                                <select
                                    value={tipoPropiedad}
                                    onChange={e => setTipoPropiedad(e.target.value)}
                                    className="w-full px-6 py-4 bg-transparent text-[#4a4a4a] text-[13px] font-sans font-bold uppercase tracking-widest focus:outline-none appearance-none cursor-pointer"
                                >
                            ...
                            {/* Precio Mínimo */}
                            <div className="flex-1 bg-[#f9f8f6] rounded-[24px] hover:bg-[#f0eee9] w-full px-5 py-2 flex flex-col justify-center transition-colors duration-300 border focus-within:border-[#a1824a] border-transparent">
                                <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-1 ml-1">Precio Mínimo (USD)</span>
                                <input
                                    type="number"
                                    placeholder="Ej. 50000"
                                    value={precioMin}
                                    onChange={e => setPrecioMin(e.target.value)}
                                    className="w-full bg-transparent text-[#4a4a4a] text-[13px] font-bold focus:outline-none ml-1 placeholder-gray-300"
                                    min="0"
                                />
                            </div>
                            ...
                            {/* Precio Máximo */}
                            <div className="flex-1 bg-[#f9f8f6] rounded-[24px] hover:bg-[#f0eee9] w-full px-5 py-2 flex flex-col justify-center transition-colors duration-300 border focus-within:border-[#a1824a] border-transparent">
                                <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-1 ml-1">Precio Máximo (USD)</span>
                                <input
                                    type="number"
